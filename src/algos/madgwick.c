...
void imu_filter(float ax, float ay, float az, float gx, float gy, float gz) {
    ...
    q_w.a = 0;                              // the real component is zero, which the Madgwick uses to simplfy quat. mult.
    q_w.b = gx;
    q_w.c = gy;
    q_w.d = gz;
    
    q_w = q_scalar_mult(&q_w, 0.5);                  // equation (12) dq/dt = (1/2)q*w
    q_w = q_mult(&q_est_prev, &q_w);        // equation (12)
    ...
    */
    ...
    q_a = q_normalize(&q_a);              // normalize the acceleration quaternion to be a unit quaternion
    ...
    F_g[0] = 2*(q_est_prev.b * q_est_prev.d - q_est_prev.a * q_est_prev.c) - q_a.b;
    F_g[1] = 2*(q_est_prev.a * q_est_prev.b + q_est_prev.c * q_est_prev.d) - q_a.c;
    F_g[2] = 2*(0.5 - q_est_prev.b * q_est_prev.b - q_est_prev.c * q_est_prev.c) - q_a.d;
    
    //Compute the Jacobian matrix, equation (26), for gravity
    J_g[0][0] = -2 * q_est_prev.c;
    J_g[0][1] =  2 * q_est_prev.d;
    J_g[0][2] = -2 * q_est_prev.a;
    J_g[0][3] =  2 * q_est_prev.b;
    
    J_g[1][0] = 2 * q_est_prev.b;
    J_g[1][1] = 2 * q_est_prev.a;
    J_g[1][2] = 2 * q_est_prev.d;
    J_g[1][3] = 2 * q_est_prev.c;
    
    J_g[2][0] = 0;
    J_g[2][1] = -4 * q_est_prev.b;
    J_g[2][2] = -4 * q_est_prev.c;
    J_g[2][3] = 0;
    
    // now computer the gradient, equation (20), gradient = J_g'*F_g
    gradient.a = J_g[0][0] * F_g[0] + J_g[1][0] * F_g[1] + J_g[2][0] * F_g[2];
    gradient.b = J_g[0][1] * F_g[0] + J_g[1][1] * F_g[1] + J_g[2][1] * F_g[2];
    gradient.c = J_g[0][2] * F_g[0] + J_g[1][2] * F_g[1] + J_g[2][2] * F_g[2];
    gradient.d = J_g[0][3] * F_g[0] + J_g[1][3] * F_g[1] + J_g[2][3] * F_g[2];
    
    // Normalize the gradient, equation (44)
    gradient = q_normalize(&gradient);
    ...
    */
    gradient = q_scalar_mult(&gradient, BETA);             // multiply normalized gradient by beta
    q_est_dot = q_sub(&q_w, &gradient);        // subtract above from q_w, the integrated gyro quaternion
    q_est_dot = q_scalar_mult(&q_est_dot, DELTA_T);
    q_est = q_sum(&q_est_prev, &q_est_dot);     // Integrate orientation rate to find position
    q_est = q_normalize(&q_est);                 // normalize the orientation of the estimate
